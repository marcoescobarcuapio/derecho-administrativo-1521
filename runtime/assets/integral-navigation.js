(() => {
  ...
  const MOTION_MS = body.dataset.unit === "1" ? 560 : 760;
  const EXIT_MS = 260;
  ...
  if (
    body.dataset.unit !== "1" || narrowQuery.matches || coarseQuery.matches ||
    lecturaParameter === "1"
  ) {
  ...
  const update = (index, focusHeading) => {
    ...
    fitSlide(current);
    if (liveStatus) {
      const title = current.querySelector("h2")?.textContent?.trim() || "Lámina";
      liveStatus.textContent = `Lámina ${activeIndex + 1} de ${slides.length}: ${title}`;
    }
    if (focusHeading) {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (control) {
      ...
      if (action === "references") goTo(slides.length - 1, true);
